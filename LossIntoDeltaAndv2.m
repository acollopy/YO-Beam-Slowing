function [XV,LostToDelta] = LossIntoDeltaAndv2(XV,vStart,angle,v2BeamSize,LostToDelta)  
    ...
    LossFractionv1 = 1*(1-exp(-(vStart-XV(6,:))/(6e-3*213));%loss int v = 1, 213 photons before going into v = 1
    InV2 = zeros(1,NumberOfMolecules) + ((XV(1,:)-tan(angle)*(XV(3,:)-.5*CellToMotDist)).^2+(XV(2,:).^2) <= v2BeamSize^2);
    StateNew = XV(9,:) | InV2; %if in v=2 beam, move back to state 1
